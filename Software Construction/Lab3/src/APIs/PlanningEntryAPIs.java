...
public class PlanningEntryAPIs<R> {
	...
	private String check(List<? extends PlanningEntry> entries) {
		if(entries.get(0) instanceof CourseEntry) {
			for(int i=0;i<entries.size();i++) {
				CourseEntry a1=new CourseEntry();
				a1=(CourseEntry) entries.get(i);
				if(a1.timeIsSet()==false) {
					continue;
				}
				for(int j=i+1;j<entries.size();++j) {
					CourseEntry a2=new CourseEntry();
					a2=(CourseEntry) entries.get(j);
	 ...
	 * @return 前序计划项
	 */
	public PlanningEntry findPreEntryPerResource(Strategy<?> strategy,R resource,PlanningEntry e,
	List<?> entries) {
